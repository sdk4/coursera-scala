...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def occurrences(chars: List[Char], acc: Map[Char, Int]): Map[Char, Int] = chars match {
      case List()       => acc
      case char :: tail => occurrences(tail, acc.updated(char, acc.withDefault(key => 0)(char) + 1))
    }
    occurrences(chars, Map[Char, Int]()).toList
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    ...
    case h1 :: h2 :: t => { 
      val newTree = makeCodeTree(h1, h2)   
      t.filter(tree => weight(tree) <= weight(newTree)) ::: newTree :: t.filter(tree => weight(tree) > weight(newTree))
    }
    case _ => trees
  }
  /**
